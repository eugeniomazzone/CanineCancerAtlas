...
if (file.exists(processed_bed)){
	...
	BINpack.Maker(bed.file = paste0('/refFiles/',bed), bin.size = 150, genome.pkg = "BSgenome.Cfamiliaris.UCSC.canFam3") ### RUN ONE TIME FOR NEW BEDFILE
}

print(paste0('Start processing: ', args[2]))
try(WES.Bin(testBAM = paste(biop,'/', args[2],'.ready.bam', sep=''), refBAM = paste(biop,'/', args[1],'.ready.bam', sep=''), BINpack = paste0(processed_bed), samplename = args[2]))
